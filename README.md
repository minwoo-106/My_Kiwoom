...
- 종목별 OpenDART 공시 위험 신호등: 양호·주의·위험·오류/지연 표시 (주문과 완전 분리)
...
공시 감시 기능은 자동 매수·매도와 연결되지 않습니다. 5종목 감시 표의 `공시` 열과 `전략 / 신호 통계`에서 종목별·전체 상태를 표시합니다.

- **양호(초록)**: 마지막 정상 공식 공시 조회에서 정의된 위험·주의 키워드가 없음
- **주의(노랑)**: 유상증자·전환사채·실적 부진·소송 등 확인이 필요한 공시
- **위험(빨강)**: 거래정지·상장폐지·파산·부도·횡령·배임·영업정지 등 중대한 위험 키워드
- **오류/지연(빨강)**: OpenDART API가 실패했거나 마지막 정상 조회가 오래됨
- **미설정(회색)**: OpenDART API 키가 아직 없음. 양호를 뜻하지 않음
...
공시 키는 [OpenDART](https://opendart.fss.or.kr/)에서 발급받습니다. `양호`는 종목 자체가 안전하다는 뜻이 아니라, 최근 공식 공시 제목에서 정의된 위험 키워드가 발견되지 않았다는 뜻입니다.
...
- 이 프로젝트는 수익을 보장하지 않는 모의 검증 도구입니다. 공시는 참고하되, 충분한 테스트 기록 없이 규칙을 바꾸지 않습니다.
